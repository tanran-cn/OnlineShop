...
from MxShop.settings import ali_pub_key_path
from MxShop.settings import private_key_path
...
class AlipayViewset(APIView):
    def get(self, request):
        ...
        alipay = AliPay(
            appid="2016091100486979",
            app_notify_url="http://47.95.201.121:8000/alipay/return/",
            app_private_key_path=private_key_path,
            alipay_public_key_path=ali_pub_key_path,  # 支付宝的公钥，验证支付宝回传消息使用，不是你自己的公钥,
            debug=True,  # 默认False,
            return_url="http://47.95.201.121:8000/alipay/return/"
        )
        varify_re = (alipay.verify(processed_dict, sign))

        if varify_re is True:
            order_sn = processed_dict.get('out_trade_no', None)
            trade_no = processed_dict.get('trade_no', None)
            trade_status = processed_dict.get('trade_status', None)

            existed_order = OrderInfo.objects.filter(order_sn=order_sn)
            for existed_order in existed_order:
